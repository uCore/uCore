...
class uCustomWidget implements iWidget {
	...
	public static function getPossibleFields($originalVal,$pk,$processedVal,$rec) {
		...
		$fields = $obj->fields;
		$ret = '';
		$ret .= '<button data-fieldname="total" class="btn widget-field">Total Records</button>';
		if (isset($rec['limit']) && strpos($rec['limit'],',')===FALSE && isset($rec['content']) && stripos($rec['content'],'{pagination}') !== FALSE) {
			$ret .= '<button data-fieldname="pages" class="btn widget-field">Total Pages</button>';
			$ret .= '<button data-fieldname="current_page" class="btn widget-field">Current Page</button>';
			$ret .= '<button data-fieldname="pagination" class="btn widget-field">Pagination</button>';
		}
		$ret .= '<button data-fieldname="field._module_url" class="btn widget-field">Module URL</button>';
		foreach ($fields as $field) {
			$visname = $field['visiblename'] ? $field['visiblename'] : $field['alias'];
			$ret .= '<button data-fieldname="field.'.$field['alias'].'" class="btn widget-field">'.$visname.'</button>';
		}
		return trim($ret);
	}
