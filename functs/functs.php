...
function array_sort_subkey(&$array,$key,$ascending=true) {
	uasort($array,array(new SubkeySorter($key),'doSort'));
	if (!$ascending) $array = array_reverse($array,true);
}
...
function UrlReadable($string) {
	return preg_replace('/\-+/','-',preg_replace('/[^\pL0-9a-z\-_~]+/iu','-',$string));
}
...
function AjaxEcho($text=null) {
	...
	if (!utopia::IsAjaxRequest()) return false;
	if (!$text) return true;
	$text = trim($text,"\n;");
	echo "\n$text;";
	return true;
